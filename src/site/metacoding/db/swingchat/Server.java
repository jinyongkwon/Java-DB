...
public class Server {
	...
	List<ClientSocket> ClientSocketList; // socket를 저장하기 위한 리스트.
	...
	public Server() {
		try {
			...
			ClientSocketList = new Vector<>(); // Vector => 동기화가 처리된 ArrayList
			while (true) {
				...
				ClientSocketList.add(t); // 리스트에 클래스를 추가 = 리스트에다 클래스에 연결된 socket을 보관
	...
	class ClientSocket implements Runnable { // socket을 받는 클래스.
		...
		public void chatPublic(String msg) {
			for (ClientSocket t : ClientSocketList) {
				try {
					if (t != this) {
			...
			while (isLogin) {
				try { // 연결해제하면 reader가 끊겨서 readLine 부분이 오류가남.
					...
					try {
						...
						ClientSocketList.remove(this); // 자기 자신을 heap에서 날려줌.
